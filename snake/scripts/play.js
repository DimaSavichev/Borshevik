...
window.onload = function() {
    ...
    let k = window.innerHeight;
    let val = n.toString() + "px";
    let vul = k.toString() +"px";
...
//основной цикл игры
function loop() {
    ctx.textContent = 'Счет: ' + score.toString();
    ctx.fillStyle = 'black';
    ...
    for( var a = 0; a < apples.length; a++ ) {
        if((px < (apples[a].x + pw)) && (px + pw > apples[a].x) && (py < (apples[a].y + ph)) && (py + ph > apples[a].y)) {
            ...
            if (opr()) {
                tail += 10; // удлиняем
                speed += 0.05; // увеличиваем скорость
                score += 10;
            }else{
                speed += 0.1;
            }
